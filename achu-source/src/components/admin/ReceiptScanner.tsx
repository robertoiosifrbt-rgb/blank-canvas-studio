...
import { AlertTriangle, FileText, X, Check, Loader2, Sparkles, AlertCircle, Info } from 'lucide-react';
import { uploadFile } from '@/lib/storage';
import { saveExpense, getJobsForSelect, extractReceipt } from '@/lib/endpoints';
import { toast } from 'sonner';
import { useDebouncedCallback } from 'use-debounce';
import { fmtDate, fmt } from '@/lib/format';
import { Card, CardContent } from '@/components/ui/card';
import CaptureStep from './ReceiptCaptureStep';
import DocumentPreview from './ReceiptDocumentPreview';
import { validateFile, isAiFilled } from '@/lib/receiptFile';
...
type JobOption = { id: string; /** numărul VIZIBIL, nu cuid-ul */ jobId?: number; customerName: string; service: string; jobDate: string };
type DuplicateMatch = { expenseId?: number; supplier?: string; expenseDate?: string; amount?: number; category?: string; linkedJobLabel?: string; documentNumber?: string };
...
export default function ReceiptScanner({ open, onClose, onSaved }: { open: boolean; onClose: () => void; onSaved: () => void }) {
  ...
  // Bare storage path to persist on the Expense record — separate from
  // `fileUrl` (a short-lived signed URL used only for the OCR call and the
  // review-step preview, never saved).
  ...
  const handleFileSelected = async (file: File) => {
    ...
    try {
      const { path, previewUrl } = await uploadFile({ data: file, filename: file.name });
      ...
      setStep('extracting');
      try {
        const result = await extractReceipt({ fileUrl: previewUrl, isPdf: isFilePdf });
        ...
        console.error('[ReceiptScanner] Extraction error:', errMsg(extractErr));
        setExtractionFailed(true);
        setExtractionError(errMsg(extractErr) ?? 'Extraction failed. Please enter details manually.');
      }

      setStep('review');
    } catch (e) {
      setError(errMsg(e) ?? 'Failed to upload file');
      setStep('capture');
    }
  };
  ...
  const searchJobs = useCallback(async (q: string) => {
    try {
      ...
      console.warn('[ReceiptScanner] Job search failed:', errMsg(err));
    }
  }, []);
  ...
  const doSave = async (overrideConfirmed: boolean) => {
    ...
    try {
      const res = await saveExpense({
        ...
        receiptFileUrl: filePathRef.current,
      ...
      // 🔴 ACHU-750 — ecranul de jos e ACCESIBIL de pe 19/08/2026 (decizie de owner); până atunci ruta
      // arunca 400, iar casta de aici era dovada. Motivul: `lib/expenseDuplicateOverride.ts`.
      if (res.duplicates?.length) {
        setDuplicates(res.duplicates); setStep('duplicate'); setSaving(false);
        return;
      }
      ...
      setError(errMsg(e) ?? 'Failed to save expense');
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-3">
              <div className="grid grid-cols-2 gap-3">
                <div><Label htmlFor="receiptsca-document-type">Document Type</Label>
                  <Select value={form.documentType} onValueChange={v => setForm(f => ({ ...f, documentType: v }))}>
                    <SelectTrigger id="receiptsca-document-type" className={fieldRing('documentType')}><SelectValue /></SelectTrigger>
                    <SelectContent>{docTypes.map(d => <SelectItem key={d.value} value={d.value}>{d.label}</SelectItem>)}</SelectContent>
                  </Select>
                </div>
                <div><Label htmlFor="receiptsca-document-number">Document Number</Label><Input id="receiptsca-document-number" className={fieldRing('documentNumber')} value={form.documentNumber} onChange={e => setForm(f => ({ ...f, documentNumber: e.target.value }))} placeholder="INV-001" /></div>
              </div>
              <div><Label htmlFor="receiptsca-supplier">Supplier *</Label><Input id="receiptsca-supplier" className={fieldRing('supplier')} value={form.supplier} onChange={e => setForm(f => ({ ...f, supplier: e.target.value }))} /></div>
              <div className="grid grid-cols-2 gap-3">
                <div><Label htmlFor="receiptsca-expense-date">Expense Date *</Label><DateField id="receiptsca-expense-date" className={fieldRing('expenseDate')} value={form.expenseDate} onChange={e => setForm(f => ({ ...f, expenseDate: e.target.value }))} /></div>
                <div><Label htmlFor="receiptsca-currency">Currency</Label><Input id="receiptsca-currency" className={fieldRing('currency')} value={form.currency} onChange={e => setForm(f => ({ ...f, currency: e.target.value }))} /></div>
              </div>
              <div className="grid grid-cols-3 gap-3">
                <div><Label htmlFor="receiptsca-subtotal">Subtotal (£)</Label><Input id="receiptsca-subtotal" className={fieldRing('subtotal')} type="number" step="0.01" value={aiFilledFields.has('subtotal') || form.subtotal !== 0 ? form.subtotal : ''} onChange={e => setForm(f => ({ ...f, subtotal: parseFloat(e.target.value) || 0 }))} /></div>
                <div><Label htmlFor="receiptsca-vat">VAT (£)</Label><Input id="receiptsca-vat" className={fieldRing('vatAmount')} type="number" step="0.01" value={aiFilledFields.has('vatAmount') || form.vatAmount !== 0 ? form.vatAmount : ''} onChange={e => setForm(f => ({ ...f, vatAmount: parseFloat(e.target.value) || 0 }))} /></div>
                <div><Label htmlFor="receiptsca-total">Total (£) *</Label><Input id="receiptsca-total" className={fieldRing('amount')} type="number" step="0.01" value={aiFilledFields.has('amount') || form.amount !== 0 ? form.amount : ''} onChange={e => setForm(f => ({ ...f, amount: parseFloat(e.target.value) || 0 }))} /></div>
              </div>
              {/* ACHU-022: Category is required — highlight if not set */}
              <div>
                <Label htmlFor="receiptsca-category">Category *</Label>
                <Select value={form.category} onValueChange={v => setForm(f => ({ ...f, category: v }))}>
                  <SelectTrigger id="receiptsca-category" className={`${fieldRing('category')} ${!form.category ? 'ring-2 ring-amber-400/50' : ''}`}>
              ...
              <div><Label htmlFor="receiptsca-payment-method">Payment Method</Label>
                <Select value={form.paymentMethod} onValueChange={v => setForm(f => ({ ...f, paymentMethod: v }))}>
                  <SelectTrigger id="receiptsca-payment-method" className={fieldRing('paymentMethod')}><SelectValue placeholder="Select" /></SelectTrigger>
                  <SelectContent>{methods.map(m => <SelectItem key={m} value={m}>{m}</SelectItem>)}</SelectContent>
                </Select>
              </div>

              {/* ACHU-523: grup (căutare, rezultat ales, buton de ștergere), nu un câmp singur —
                  iar controlul vizibil se SCHIMBĂ după stare, deci `htmlFor` ar rămâne suspendat. */}
              <div role="group" aria-labelledby="scanner-linked-job-label">
                <Label id="scanner-linked-job-label">Linked Job (optional)</Label>
                {selectedJob ? (
                  <div className="flex items-center gap-2 p-2 border border-border rounded-lg bg-muted/30">
                    ...
                    <button aria-label={`Unlink job #${selectedJob.jobId} from this receipt`} title={`Unlink job #${selectedJob.jobId} from this receipt`} onClick={() => { setSelectedJob(null); setForm(f => ({ ...f, linkedJob: '' })); }} className="p-1"><X className="h-3.5 w-3.5" /></button>
                  </div>
                ) : showJobSearch ? (
                  <div className="space-y-1">
                    <Input aria-label="Search jobs" placeholder="Search jobs..." value={jobSearch} onChange={e => { setJobSearch(e.target.value); debouncedJobSearch(e.target.value); }} autoFocus />
              ...
              <div><Label htmlFor="receiptsca-description-items">Description / Items</Label><Textarea id="receiptsca-description-items" className={fieldRing('description')} value={form.description} onChange={e => setForm(f => ({ ...f, description: e.target.value }))} rows={2} /></div>
              <div><Label htmlFor="receiptsca-notes">Notes</Label><Textarea id="receiptsca-notes" value={form.notes} onChange={e => setForm(f => ({ ...f, notes: e.target.value }))} rows={2} /></div>
            </div>
