...
import { getCleaners, type CleanerRecord } from '@/lib/endpoints';
...
const SORT_FIELDS: SortField<CleanerRecord>[] = [
...
export default function CleanersPage() {
  const req = useTrackedRequest<{ records: CleanerRecord[] }>({ timeoutMs: 30000 });
  const records = req.data?.records ?? [];
  const [search, setSearch] = useState('');
  const [editItem, setEditItem] = useState<CleanerRecord | null>(null);
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3">
        <div className="relative max-w-sm flex-1 w-full sm:w-auto">
          ...
          <Input aria-label="Search cleaners" placeholder="Search cleaners..." className="pl-9" value={search} onChange={e => { setSearch(e.target.value); onSearch(e.target.value); }} />
        </div>
          ...
          <tbody>
            {showSkeleton ? Array.from({ length: 3 }).map((_, i) => (
              ...
              <tr key={r.id} className="border-t border-border hover:bg-muted/30">
                ...
                <td className="p-3"><button aria-label={`Edit ${r.cleanerName}`} title={`Edit ${r.cleanerName}`} className="p-1.5 rounded hover:bg-muted" onClick={() => { setEditItem(r); setDialogOpen(true); }}><Pencil className="h-3.5 w-3.5" /></button></td>
              </tr>
            ))}
          </tbody>
        </table>
