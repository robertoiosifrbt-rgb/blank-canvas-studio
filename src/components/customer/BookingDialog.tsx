...
import { requestBooking, getCustomerPortal } from 'zite-endpoints-sdk';
...
export default function BookingDialog({ open, onClose, address, onSaved }: {
  ...
  const [success, setSuccess] = useState<{ jobId: any } | null>(null);
  ...
  const reconcileBeforeRetry = async () => {
    ...
    try {
      ...
      const recentEnquiry = fresh.upcomingJobs?.find((j: any) =>
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setErrors([e?.message || 'Failed to submit booking request. Please try again.']);
      ...
      <DialogContent className="max-w-md max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-3">
          ...
          <div>
            ...
            <Input type="date" min={todayStr} value={form.jobDate} onChange={e => setForm(f => ({ ...f, jobDate: e.target.value }))} disabled={saving} />
          </div>
          <div>
            <Label>Preferred Start Time (optional)</Label>
            <Input type="time" value={form.startTime} onChange={e => setForm(f => ({ ...f, startTime: e.target.value }))} disabled={saving} />
          </div>
          <div>
            <Label>Address *</Label>
            <Textarea value={form.address} onChange={e => setForm(f => ({ ...f, address: e.target.value }))} rows={2} disabled={saving} />
          </div>
          <div>
            <Label>Customer Instructions</Label>
            <Textarea value={form.notes} onChange={e => setForm(f => ({ ...f, notes: e.target.value }))} rows={2} placeholder="Access codes, parking, special requests..." disabled={saving} />
          </div>
          <div>
            <Label>Additional Details</Label>
            <Textarea value={form.additionalDetails} onChange={e => setForm(f => ({ ...f, additionalDetails: e.target.value }))} rows={2} placeholder="Anything else we should know..." disabled={saving} />
          </div>
