...
import { getActionCentre, GetActionCentreOutputType, convertQuoteRequest } from 'zite-endpoints-sdk';
...
import {
  Search, RefreshCw, AlertCircle, Briefcase, PoundSterling,
  RotateCcw, XCircle, Receipt, ChevronRight, ArrowLeft, ChevronDown, CalendarDays, FileText, UserCog, CheckCircle2,
} from 'lucide-react';

type SectionKey = 'jobs' | 'money' | 'refunds' | 'cancelled' | 'preparedAccounts' | 'expenses' | 'conversions' | 'futureJobs' | 'completedJobs';
...
const SECTION_META: Record<SectionKey, { label: string; icon: any; description: string }> = {
  ...
  refunds: { label: 'Refunds & Corrections', icon: RotateCcw, description: 'Overpayments, voided and flagged payments' },
...
const ACTION_SECTION_KEYS: Array<'jobs' | 'money' | 'refunds' | 'cancelled' | 'expenses' | 'conversions'> = ['jobs', 'money', 'refunds', 'cancelled', 'expenses', 'conversions'];

// ACHU-135: Card layout rows for the overview grid
const CARD_ROW_1: SectionKey[] = ['jobs', 'money', 'refunds'];
const CARD_ROW_2: SectionKey[] = ['cancelled']; // futureJobs + completedJobs are custom cards in this row
const CARD_ROW_3: SectionKey[] = ['preparedAccounts', 'expenses', 'conversions'];

const SECTION_COLORS: Record<SectionKey, { bg: string; border: string; activeBg: string; badge: string; icon: string; text: string }> = {
...
export default function ActionCentrePage() {
  ...
  const load = useCallback(() => {
    actReq.fire(async () => {
      ...
      try {
        ...
        setConversionSummary(`Conversion error: ${convErr?.message || 'Unknown error'}`);
      }
  ...
  const openRecord = (item: ActionItem) => {
    ...
    if (item.entityType === 'job') nav(`/admin/jobs?id=${item.entityId}&returnTo=${returnTo}`);
    else if (item.entityType === 'payment') nav(`/admin/payments?id=${item.entityId}&returnTo=${returnTo}`);
    else if (item.entityType === 'expense') nav(`/admin/expenses?id=${item.entityId}&returnTo=${returnTo}`);
    else if (item.entityType === 'userAccount') nav(`/admin/users?id=${item.entityId}&returnTo=${returnTo}`);
    else if (item.entityType === 'quoteRequest') nav(`/admin/quote-requests?id=${item.entityId}&returnTo=${returnTo}`);
  };
  ...
  const grandTotal = ACTION_SECTION_KEYS.reduce((s, k) => s + data[k].totalCount, 0);
      ...
      <div className="space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div className="flex items-center gap-2">
            <Button variant="ghost" size="icon" onClick={() => nav('/admin')} className="shrink-0">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {CARD_ROW_1.map(k => (
            <SectionCard key={k} sectionKey={k} data={(data as any)[k]} active={false} onClick={() => setSection(k)} />
          ))}
        </div>
              ...
              {sd.categories.length > 0 && (
                <div className="flex flex-wrap gap-1.5 pl-6">
                  {sd.categories.map(c => (
                    <button
                      key={c.key}
                      onClick={() => { setSection(k); setTimeout(() => setFilter(c.key), 0); }}
  ...
  const sectionData = (data as any)[section] as ActionSectionData;
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div className="flex items-center gap-2 min-w-0">
          <Button variant="ghost" size="icon" onClick={() => setSection('' as any)} className="shrink-0">
            <ArrowLeft className="h-4 w-4" />
          </Button>
      ...
      <div className="relative max-w-sm">
        ...
        <Input placeholder="Search within results…" className="pl-9" value={search} onChange={e => setSearch(e.target.value)} />
      </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-muted/50">
                  ...
                  <th className="text-left p-3 font-medium">{section === 'expenses' ? 'Supplier' : 'Customer'}</th>
              ...
              <tbody>
                {items.map(item => (
                  ...
                  >
                    ...
                    <td className="p-3">{item.customerName ?? item.supplier ?? '—'}</td>
...
function CompletedJobsSummaryCard({ data, onClick }: {
...
type CompletedJob = ActionData['completedJobs']['items'][0];

function CompletedJobsView({ data, search, setSearch, onBack, onRefresh, refreshing, openJob, error }: {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="relative max-w-sm">
        <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" />
        <Input placeholder="Search jobs, customers, cleaners…" className="pl-9" value={search} onChange={e => setSearch(e.target.value)} />
      </div>
        ...
        <>
          ...
          <div className="hidden md:block rounded-lg border border-border overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-muted/50">
                  <th className="text-left p-2.5 font-medium text-xs">Job</th>
                  <th className="text-left p-2.5 font-medium text-xs">Customer</th>
                  <th className="text-left p-2.5 font-medium text-xs">Service</th>
                  <th className="text-left p-2.5 font-medium text-xs">Address</th>
                  <th className="text-left p-2.5 font-medium text-xs">Assigned</th>
                  <th className="text-left p-2.5 font-medium text-xs">Job Date</th>
                  <th className="text-left p-2.5 font-medium text-xs">Finish Time</th>
                  <th className="text-right p-2.5 font-medium text-xs">Charged</th>
                  <th className="text-left p-2.5 font-medium text-xs">Payment</th>
