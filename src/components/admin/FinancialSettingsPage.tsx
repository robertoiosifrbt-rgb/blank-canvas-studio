...
import { getFinancialSettings, saveFinancialSettings } from 'zite-endpoints-sdk';
    ...
    <div className="space-y-4 max-w-2xl">
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div><Label>Tax Reserve %</Label><Input type="number" step="0.1" min="0" max="100" value={form.taxReserve} onChange={e => setForm(f => ({ ...f, taxReserve: parseFloat(e.target.value) || 0 }))} /></div>
            <div><Label>NI Reserve %</Label><Input type="number" step="0.1" min="0" max="100" value={form.nationalInsuranceReserve} onChange={e => setForm(f => ({ ...f, nationalInsuranceReserve: parseFloat(e.target.value) || 0 }))} /></div>
            <div><Label>Emergency Reserve %</Label><Input type="number" step="0.1" min="0" max="100" value={form.emergencyReserve} onChange={e => setForm(f => ({ ...f, emergencyReserve: parseFloat(e.target.value) || 0 }))} /></div>
          </div>
            ...
            <>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <Input type="date" value={form.taxYearStart} onChange={e => setForm(f => ({ ...f, taxYearStart: e.target.value }))} />
                </div>
                <div>
                  <Label>Tax Year End</Label>
                  <Input type="date" value={form.taxYearEnd} onChange={e => setForm(f => ({ ...f, taxYearEnd: e.target.value }))} />
                </div>
              </div>
              {dateErrors.length > 0 && (
      ...
      <Card>
        <CardContent className="space-y-4 pt-6">
          <div><Label>Notes</Label><Textarea value={form.notes} onChange={e => setForm(f => ({ ...f, notes: e.target.value }))} rows={3} maxLength={LIMITS.notes} /></div>
