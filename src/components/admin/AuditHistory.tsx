...
import { getAuditHistory, GetAuditHistoryOutputType } from 'zite-endpoints-sdk';
import { Button } from '@/components/ui/button';
import { Input } from '@/components/ui/input';
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from '@/components/ui/select';
import { Label } from '@/components/ui/label';
import { Card, CardContent } from '@/components/ui/card';
import { ChevronDown, ChevronUp, History, Loader2, Search, RefreshCw, AlertCircle } from 'lucide-react';
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between gap-3">
        <h2 className="text-2xl font-bold flex items-center gap-2"><History className="h-6 w-6" />Audit History</h2>
      ...
      <div className="grid grid-cols-2 md:grid-cols-6 gap-3">
        ...
        <div>
          ...
          <Input className="h-8 text-sm" placeholder="Record ID..." value={entityId} onChange={e => setEntityId(e.target.value)} />
        </div>
        <div>
          <Label className="text-xs">Performed By</Label>
          <Input className="h-8 text-sm" placeholder="Email..." value={performedBy} onChange={e => setPerformedBy(e.target.value)} />
        </div>
        <div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-muted/50">
                  ...
                  <th className="text-left p-3 font-medium hidden lg:table-cell">By</th>
                  <th className="text-left p-3 font-medium hidden lg:table-cell">Changes</th>
                </tr>
              </thead>
              <tbody>
                {events.map(ev => (
